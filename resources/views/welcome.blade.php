...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .casino-wrap::before {
    ...
    height: 3px;
    background: linear-gradient(90deg, #ff6b00, #ffd700, #ff6b00, #ffd700);
  ...
  .reel-window::before { top: 0; background: linear-gradient(#080815, transparent); }
  .reel-window::after { bottom: 0; background: linear-gradient(transparent, #080815); }
  ...
  .center-line {
    ...
    height: 46px;
    border-top: 1.5px solid #ffd70040;
    border-bottom: 1.5px solid #ffd70040;
    pointer-events: none;
    z-index: 3;
  }
  ...
  .win-flash .reel-wrapper { animation: winBorder 0.4s ease 3; }
  @keyframes winBorder {
    0%, 100% { border-color: #2a2a50; }
    50% { border-color: #ffd700; box-shadow: 0 0 12px #ffd70060; }
  }
  ...
  .stat-value.bet { color: #88aaff; }
  .stat-value.win { color: #66ee99; }
  ...
  .bet-btn:hover { background: #252545; color: #fff; }
  ...
  .spin-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 4px 16px #ff6b0060; }
  ...
  .max-btn:hover { background: #252545; }
  ...
  .message-bar.win { color: #66ee99; background: #66ee991a; border: 1px solid #66ee9930; }
  .message-bar.jackpot { color: #ffd700; background: #ffd7001a; border: 1px solid #ffd70030; animation: pulse 0.5s ease infinite alternate; }
  .message-bar.lose { color: #ff5555; background: transparent; }
  ...
  .paytable-toggle button:hover { color: #888; }
  ...
  .pt-mult { color: #ffd700; font-weight: 700; font-size: 13px; margin-left: auto; }
  ...
  .light.on { background: #ffd700; box-shadow: 0 0 6px #ffd700; }
  .light.on-r { background: #ff4444; box-shadow: 0 0 6px #ff4444; }
</style>

<body>
<div class="casino-wrap">
 
  <div class="lights-row" id="lights">
    <div class="light"></div>
    <div class="light"></div>
    <div class="light"></div>
    <div class="light"></div>
  ...
  <div class="machine-body">
    ...
    <div class="info-bar">
      <div class="stat-box">
        ...
        <div class="stat-value coins" id="coins-display">500</div>
      </div>
      <div class="stat-box">
        <div class="stat-label">Taruhan</div>
        <div class="stat-value bet" id="bet-display">10</div>
      </div>
      <div class="stat-box">
        <div class="stat-label">Menang</div>
        <div class="stat-value win" id="win-display">0</div>
      </div>
    </div>
    ...
    <div class="controls">
      <div class="bet-control">
        ...
        <span class="bet-amount" id="bet-val">10</span>
  ...
  <div class="message-bar" id="msg">Tekan SPIN untuk mulai!</div>

  <div class="paytable-toggle">
    <button id="pt-toggle">lihat tabel pembayaran</button>
  </div>

  <div class="paytable" id="paytable">
    <div class="pt-title">Tabel Pembayaran</div>
...
<script>
const SYMBOLS = [
  { sym: '🍒', name: 'Ceri',    mult: 2  },
  { sym: '🍋', name: 'Lemon',   mult: 3  },
  { sym: '🍊', name: 'Jeruk',   mult: 4  },
  { sym: '🍇', name: 'Anggur',  mult: 5  },
  { sym: '🔔', name: 'Bel',     mult: 8  },
  { sym: '⭐', name: 'Bintang', mult: 10 },
  { sym: '💎', name: 'Berlian', mult: 15 },
  { sym: '7️⃣', name: 'Lucky 7', mult: 25 },
];
...
const reelStrips = Array.from({length: REELS}, () => {
  let strip = [];
  for (let i = 0; i < STRIP_LEN; i++) {
    let r = Math.random();
    if (r < 0.05) strip.push(7);
    else if (r < 0.1) strip.push(6);
    else if (r < 0.18) strip.push(5);
    else if (r < 0.28) strip.push(4);
    else if (r < 0.42) strip.push(3);
    else if (r < 0.58) strip.push(2);
    else if (r < 0.75) strip.push(1);
    else strip.push(0);
  }
  return strip;
});
...
function buildReels() {
  for (let r = 0; r < REELS; r++) {
    const strip = document.querySelectorAll('.reel-strip')[r];
    const symbols = [...reelStrips[r], ...reelStrips[r], ...reelStrips[r]];
    strip.innerHTML = symbols.map(i => `<div class="reel-symbol">${SYMBOLS[i].sym}</div>`).join('');
    setReelPos(r, reelPositions[r], false);
  }
}

function setReelPos(r, pos, animate) {
  const strip = document.querySelectorAll('.reel-strip')[r];
  const offset = -(pos * SYM_H) + SYM_H;
  if (!animate) strip.style.transition = 'none';
  strip.style.transform = `translateY(${offset}px)`;
}

function getSymbolAt(r, pos) {
  const idx = ((pos % STRIP_LEN) + STRIP_LEN) % STRIP_LEN;
  return reelStrips[r][idx];
}
...
function buildPaytable() {
  ...
  grid.innerHTML = SYMBOLS.map(s =>
    `<div class="pt-row">
      ...
      <span>${s.name}</span>
      <span class="pt-mult">×${s.mult}</span>
    </div>`
  ).join('');
}

function animateLights(mode) {
  ...
  lightInterval = setInterval(() => {
    lights.forEach((l, i) => {
      l.className = 'light';
      if (mode === 'spin') {
        if ((i + t) % 3 === 0) l.className = 'light on';
      } else if (mode === 'win') {
        if ((i + t) % 2 === 0) l.className = 'light on';
      } else if (mode === 'jackpot') {
        l.className = t % 2 === 0 ? 'light on' : 'light on-r';
      } else {
        if (i === t % lights.length) l.className = 'light on';
      }
    });
    t++;
  }, mode === 'jackpot' ? 80 : 150);
...
function idleLights() {
  ...
  const lights = document.querySelectorAll('.light');
  lightInterval = setInterval(() => {
    lights.forEach((l, i) => { l.className = 'light'; });
    lights[t % lights.length].className = 'light on';
    t++;
  }, 400);
}
...
function spin() {
  ...
  const targets = Array.from({length: REELS}, (_, r) =>
    Math.floor(Math.random() * STRIP_LEN)
  );

  const durations = [600, 900, 1200];
  const fps = 60;
  const speeds = [3, 3.5, 2.8];

  let done = 0;

  reelStrips.forEach((_, r) => {
    const strip = document.querySelectorAll('.reel-strip')[r];
    strip.style.transition = 'none';
    const start = reelPositions[r];
    const totalSteps = Math.round(durations[r] / (1000 / fps));
    let step = 0;
    const stopPos = targets[r];

    function tick() {
      step++;
      const progress = step / totalSteps;
      const ease = progress < 0.7 ? 1 : 1 - Math.pow((progress - 0.7) / 0.3, 2);
      const velocity = speeds[r] * ease;
      reelPositions[r] = (reelPositions[r] + velocity + STRIP_LEN) % STRIP_LEN;

      if (step < totalSteps) {
        const offset = -(reelPositions[r] * SYM_H) + SYM_H;
        strip.style.transform = `translateY(${offset}px)`;
        requestAnimationFrame(tick);
      } else {
        reelPositions[r] = stopPos;
        const offset = -(stopPos * SYM_H) + SYM_H;
        strip.style.transform = `translateY(${offset}px)`;
        done++;
        if (done === REELS) finishSpin(targets);
      }
    }
    requestAnimationFrame(tick);
  });
...
function finishSpin(targets) {
  const results = targets.map((t, r) => getSymbolAt(r, t));
  ...
  if (allSame) {
    ...
    const isJackpot = results[0] === 7;
    winAmount = bet * sym.mult * (isJackpot ? 3 : 1);
    if (isJackpot) {
      msgText = `🎰 JACKPOT! +${winAmount} koin!`;
      msgCls = 'jackpot';
      animateLights('jackpot');
    } else {
      msgText = `🎉 Menang! ${sym.sym}${sym.sym}${sym.sym} — +${winAmount} koin!`;
      msgCls = 'win';
      animateLights('win');
    }
    document.getElementById('reels-container').classList.add('win-flash');
    setTimeout(() => document.getElementById('reels-container').classList.remove('win-flash'), 1200);
  } else if (twoSame) {
    winAmount = Math.round(bet * 0.5);
    msgText = `✨ Dua sama — +${winAmount} koin`;
    msgCls = 'win';
  } else {
    msgText = coins === 0 ? 'Koin habis! Klik RESET' : 'Tidak menang. Coba lagi!';
    msgCls = 'lose';
    idleLights();
  }
  ...
  updateDisplay();
  showMsg(msgText, msgCls);

  if (!allSame) idleLights();
  if (allSame) setTimeout(idleLights, 3000);

  spinning = false;
  document.getElementById('spin-btn').disabled = false;
  document.getElementById('spin-btn').textContent = coins === 0 ? 'RESET' : 'SPIN';
...
document.getElementById('bet-up').addEventListener('click', () => {
  if (bet < 100 && bet < coins) {
    bet = Math.min(bet + 10, 100, coins);
...
document.getElementById('pt-toggle').addEventListener('click', () => {
  ...
  btn.textContent = pt.classList.contains('open') ? 'sembunyikan tabel' : 'lihat tabel pembayaran';
});
